...
val creditDf2 = spark.read.option("header","true").
    option("inferSchema","true").
    format("csv").
    load("/user/xw2447/project/MyData_6t.csv")
...
val featureDf2 = assembler.transform(creditDf2)
...
val labelDf2 = indexer.fit(featureDf2).transform(featureDf2)

val seed = 5043
// val Array(trainingData, testData) = labelDf.randomSplit(Array(0.7, 0.3), seed)
val trainingData=labelDf
val testData=labelDf2
...
val dft = spark.read.option("header","true").
 option("inferSchema","true").
 format("csv").
 load("/user/xw2447/project/MyData_6t.csv")
...
val df2= dft.drop("time")
// Convert 'variety' data type to Double
val toDouble = udf[Double, Int]( _.toDouble)
val df11 = df1.withColumn("variety", toDouble(df("variety")))
// Replace Nan value with mean of the columns
val imputer = new Imputer().setInputCols(df11.columns).setOutputCols(df11.columns.map(c => s"${c}")).setStrategy("mean")

val dfClean = imputer.fit(df11).transform(df11)
...
df2.coalesce(1).write.csv("/user/xw2447/project/cleaned2")
...
// val rdd = sc.textFile("/user/xw2447/project/cleaned2").map(line => line.split(","))
...
// //projected.coalesce(1).saveAsTextFile("/user/xw2447/out3/")
// println("After running dataset through PCA2")
// projected.take(10).foreach(println)
// MLUtils.saveAsLibSVMFile(projected.coalesce(1), "/user/xw2447/project/pca_op2")
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
println("The Noise reduction using PCA Executing")
...
val testData=spark.read.format("libsvm").load("/user/xw2447/project/MyData_6vlib2.txt")
