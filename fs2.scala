...
val MPCclassifier = new MultilayerPerceptronClassifier()
  .setLayers(layers)
  .setBlockSize(128)
  .setSeed(1234L)
  .setMaxIter(100)
val MPCmodel = MPCclassifier.fit(trainingData)

// run model with test data set to get predictions
// this will add new columns rawPrediction, probability and prediction
val predictionDf = MPCmodel.transform(testData)
...
val PCA_num: Int = 7
val pca = new PCA(PCA_num).fit(data.map(_.features))
...
val layers = Array[Int](PCA_num, 5, 4, 2)
// train Random Forest model with training data set
val MPCclassifier = new MultilayerPerceptronClassifier()
  .setLayers(layers)
  .setBlockSize(128)
...
val pipeline = new Pipeline().setStages(Array(labelIndexer, featureIndexer, MPCclassifier, labelConverter))
